          ...
          {posts.map((post) => (
            <article key={post.id} className="bg-brand-charcoal rounded-[2.5rem] overflow-hidden hover:shadow-xl hover:shadow-brand-lavender/10 transition-all duration-300 border border-gray-800 flex flex-col group h-full hover:-translate-y-1">
              <div className="aspect-[16/9] w-full overflow-hidden relative">
                <img src={post.image} alt={`${post.title} - Designer Cases`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" width="600" height="400" loading="lazy" />
              ...
              <div className="p-8 flex flex-col flex-1">
                ...
                <Link to={`/blog/${post.id}`} className="inline-flex items-center font-display font-semibold text-white group-hover:text-brand-lavender transition-colors mt-auto w-fit border-b-2 border-transparent group-hover:border-brand-lavender pb-1">
                  Read Article
